import
  org.lwjgl.sdl.*,
  SDLInit.*, SDLError.*, SDLVideo.*, SDLEvents.*,
  SDLRender.*, SDLPixels.*, SDLProperties.*,
  SDLHints.*
import org.lwjgl.system.*,
  MemoryUtil.*, MemoryStack.stackPush
...
def main(): Unit =
  ...
  Using.resources(stackPush(), SDL_Event.create()): (stack, event) =>
    ...
    SDL_CreateWindowAndRenderer("Hello sdl!", 640, 480, 0, windowPtr, rendererPtr)
