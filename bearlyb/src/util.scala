...
object MallocMany:
  ...
  given [N <: Int, TailMalloc <: MallocMany[N]] => (tailMalloc: TailMalloc) => MallocMany[S[N]]:
    type Return[+T] = T *: tailMalloc.Return[T]

    def mallocMany(stack: MemoryStack): Return[ByteBuffer] =
      stack.malloc(1) *: tailMalloc.mallocMany(stack)
    def mallocManyInt(stack: MemoryStack): Return[IntBuffer] =
      stack.mallocInt(1) *: tailMalloc.mallocManyInt(stack)
    def mallocManyLong(stack: MemoryStack): Return[LongBuffer] =
      stack.mallocLong(1) *: tailMalloc.mallocManyLong(stack)
    def mallocManyFloat(stack: MemoryStack): Return[FloatBuffer] =
      stack.mallocFloat(1) *: tailMalloc.mallocManyFloat(stack)
    def mallocManyDouble(stack: MemoryStack): Return[DoubleBuffer] =
      stack.mallocDouble(1) *: tailMalloc.mallocManyDouble(stack)
  end given

end MallocMany

private[bearlyb] def mallocMany(n: Int, stack: MemoryStack)(using malloc: MallocMany[n.type]): malloc.Return[ByteBuffer] =
  malloc.mallocMany(stack)
private[bearlyb] def mallocManyInt(n: Int, stack: MemoryStack)(using malloc: MallocMany[n.type]): malloc.Return[IntBuffer] =
  malloc.mallocManyInt(stack)
private[bearlyb] def mallocManyLong(n: Int, stack: MemoryStack)(using malloc: MallocMany[n.type]): malloc.Return[LongBuffer] =
  malloc.mallocManyLong(stack)
private[bearlyb] def mallocManyFloat(n: Int, stack: MemoryStack)(using malloc: MallocMany[n.type]): malloc.Return[FloatBuffer] =
  malloc.mallocManyFloat(stack)
private[bearlyb] def mallocManyDouble(n: Int, stack: MemoryStack)(using malloc: MallocMany[n.type]): malloc.Return[DoubleBuffer] =
