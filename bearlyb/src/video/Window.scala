...
class Window private (private[bearlyb] val pWindow: Long, private var windowShape: Option[Surface] = None):
  def title: String =
  ...
  def position: (x: Int, y: Int) =
    Using(stackPush()): stack =>
      ...
      SDL_GetWindowPosition(pWindow, px, py)
        .sdlErrorCheck()
      (px.get(0), py.get(0))
    .get
  ...
  def id: WindowID =
    SDL_GetWindowID(pWindow) match
      case 0 => sdlError()
      case id => id

  def icon_=(icon: Surface): Unit =
    SDL_SetWindowIcon(pWindow, icon.internal).sdlErrorCheck()
  ...
  def fullscreenMode: Option[DisplayMode] =
    Option(SDL_GetWindowFullscreenMode(pWindow))
      .map(DisplayMode.fromInternal)

  def fullscreenMode_=(mode: DisplayMode): Unit =
    SDL_SetWindowFullscreenMode(pWindow, mode.internal)
      .sdlErrorCheck()
  ...
  def popup(xOffset: Int, yOffset: Int, width: Int, height: Int, flags: Window.Flag*): Window =
    new Window(
      SDL_CreatePopupWindow(pWindow, xOffset, yOffset, width, height, SDL_WINDOW_POPUP_MENU | flags.combine)
        .sdlCreationCheck()
    )

  def tooltip(xOffset: Int, yOffset: Int, width: Int, height: Int, flags: Window.Flag*): Window =
    new Window(
      SDL_CreatePopupWindow(pWindow, xOffset, yOffset, width, height, SDL_WINDOW_TOOLTIP | flags.combine)
    )
  ...
  def surface: Surface =
    Surface.fromInternal(SDL_GetWindowSurface(pWindow))

  def presentSurface(): Unit =
    SDL_UpdateWindowSurface(pWindow).sdlErrorCheck()
  ...
  def aspectRatio: (minAspect: Float, maxAspect: Float) =
    Using(stackPush()): stack => 
      ...
      SDL_GetWindowAspectRatio(pWindow, minAspect, maxAspect)
        .sdlErrorCheck()
  ...
  def aspectRatio_=(aspectRatios: (minAspect: Float, maxAspect: Float)): Unit =
    ...
    SDL_SetWindowAspectRatio(pWindow, minAspect, maxAspect)
      .sdlErrorCheck()

  def flash(operation: FlashOperation): Unit =
    SDL_FlashWindow(pWindow, operation.internal)
      .sdlErrorCheck()

  def getDisplay: DisplayID =
    SDL_GetDisplayForWindow(pWindow)

  def vsync: Int =
    Using(stackPush()): stack =>
      val psync = stack.mallocInt(1)
      SDL_GetWindowSurfaceVSync(pWindow, psync)
        .sdlErrorCheck(psync.get(0))
    .get
  ...
  def opacity: Float =
    SDL_GetWindowOpacity(pWindow) match
      case -1.0f => sdlError()
      case o => o

  def opacity_=(opacity: Float): Unit =
    SDL_SetWindowOpacity(pWindow, opacity)
      .sdlErrorCheck()

  def mouseGrabbed: Boolean =
    SDL_GetWindowMouseGrab(pWindow)

  def mouseGrabbed_=(grabbed: Boolean): Unit =
    SDL_SetWindowMouseGrab(pWindow, grabbed)
      .sdlErrorCheck()

  def keyboardGrabbed: Boolean =
    SDL_GetWindowKeyboardGrab(pWindow)
  
  def keyboardGrabbed_=(grabbed: Boolean): Unit =
    SDL_SetWindowKeyboardGrab(pWindow, grabbed)
      .sdlErrorCheck()
  ...
  def show(): Unit =
    SDL_ShowWindow(pWindow).sdlErrorCheck()
  def hide(): Unit =
    SDL_HideWindow(pWindow).sdlErrorCheck()
  def raise(): Unit =
    SDL_RaiseWindow(pWindow).sdlErrorCheck()
  def maximize(): Unit =
    SDL_MaximizeWindow(pWindow).sdlErrorCheck()
  def minimize(): Unit =
    SDL_MinimizeWindow(pWindow).sdlErrorCheck()
  def restore(): Unit =
    SDL_RestoreWindow(pWindow).sdlErrorCheck()
  def sync(): Unit =
    SDL_SyncWindow(pWindow).sdlErrorCheck()
...
object Window:
  ...
  object Flag:
    ...
    extension (internal: Long)
      private[bearlyb] def fromInternal: Flag =
        Flag.values.find(_.internal == internal).get
  end Flag
  ...
  enum Property(val name: String):
    ...
    case AlwaysOnTop(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_ALWAYS_ON_TOP_BOOLEAN)
    /** : true if the window has no window decoration */
    case BorderlessBoolean(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_BORDERLESS_BOOLEAN)
    ...
    // case ConstrainPopup(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_CONSTRAIN_POPUP_BOOLEAN) // TODO: This seems to be missing from lwjgl?
    /** : true if the window will be used with an externally managed graphics context. */
    case ExternalGraphicsContext(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_EXTERNAL_GRAPHICS_CONTEXT_BOOLEAN)
    /** : true if the window should accept keyboard input (defaults true) */
    case Focusable(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_FOCUSABLE_BOOLEAN)
    /** : true if the window should start in fullscreen mode at desktop resolution */
    case Fullscreen(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_FULLSCREEN_BOOLEAN)
    /** : the height of the window */
    case Height(value: Long) extends Property(SDL_PROP_WINDOW_CREATE_HEIGHT_NUMBER)
    /** : true if the window should start hidden */
    case Hidden(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_HIDDEN_BOOLEAN)
    /** : true if the window uses a high pixel density buffer if possible */
    case HighPixelDensity(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_HIGH_PIXEL_DENSITY_BOOLEAN)
    /** : true if the window should start maximized */
    case Maximized(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_MAXIMIZED_BOOLEAN)
    /** : true if the window is a popup menu */
    case PopupMenu(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_MENU_BOOLEAN)
    /** : true if the window will be used with Metal rendering */
    case Metal(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_METAL_BOOLEAN)
    /** : true if the window should start minimized */
    case Minimized(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_MINIMIZED_BOOLEAN)
    /** : true if the window is modal to its parent */
    case Modal(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_MODAL_BOOLEAN)
    /** : true if the window starts with grabbed mouse focus */
    case MouseGrabbed(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_MOUSE_GRABBED_BOOLEAN)
    /** : true if the window will be used with OpenGL rendering */
    case OpenGL(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_OPENGL_BOOLEAN)
    /** : an SDL_Window that will be the parent of this window, required for windows with the "tooltip", "menu", and "modal" properties */
    case Parent(value: Window) extends Property(SDL_PROP_WINDOW_CREATE_PARENT_POINTER)
    /** : true if the window should be resizable */
    case Resizable(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_RESIZABLE_BOOLEAN)
    /** : the title of the window, in UTF-8 encoding */
    case Title(value: String) extends Property(SDL_PROP_WINDOW_CREATE_TITLE_STRING)
    /** : true if the window show transparent in the areas with alpha of 0 */
    case Transparent(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_TRANSPARENT_BOOLEAN)
    /** : true if the window is a tooltip */
    case Tooltip(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_TOOLTIP_BOOLEAN)
    /** : true if the window is a utility window, not showing in the task bar and window list */
    case Utility(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_UTILITY_BOOLEAN)
    /** : true if the window will be used with Vulkan rendering */
    case Vulkan(value: Boolean) extends Property(SDL_PROP_WINDOW_CREATE_VULKAN_BOOLEAN)
    /** : the width of the window */
    case Width(value: Long) extends Property(SDL_PROP_WINDOW_CREATE_WIDTH_NUMBER)
