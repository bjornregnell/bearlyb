...
class Surface private (
    private[bearlyb] val internal: SDL_Surface,
    private val imageData: Option[ByteBuffer] = None):
  import Surface.{PixelData, Pos}

  def pixFormat = SDL_GetPixelFormatDetails(internal.format).sdlCreationCheck()
  ...
  def destroy(): Unit =
    ...
    if internal.refcount == 0 then imageData.foreach(stbi_image_free)
  ...
  def apply[T: PixelData as pix](pos: Pos): Color[T] = pix.get(this)(pos)
  ...
  def update[T: PixelData as pix](pos: Pos, color: Color[T]): Unit = pix
    .put(this)(pos, color)

  def update[T: PixelData as pix](x: Int, y: Int, color: Color[T]): Unit = pix
    .put(this)((x, y), color)

  def mapRGBA[T: ([x] =>> Integral[x] | Fractional[x]) as num](color: Color[T])
      : Surface.RawColor =
    def toByte(x: T): Byte = num match
      case int: Integral[T]    => int.toInt(x).toByte
      case frac: Fractional[T] => frac.toInt(frac.div(x, frac.one)).toByte

    val (r, g, b, a) = color
    val result       =
      SDL_MapSurfaceRGBA(internal, toByte(r), toByte(g), toByte(b), toByte(a))
  ...
  /** Converts a color to the raw value, depending on the surface's format.
    ...
    */
  def mapRGB[T: ([x] =>> Integral[x] | Fractional[x]) as num](
      color: NamedTuple.Init[Color[T]]
    ): Surface.RawColor =
    def toByte(x: T): Byte = num match
      case int: Integral[T]    => int.toInt(x).toByte
      case frac: Fractional[T] => frac.toInt(frac.times(x, frac.fromInt(255)))
          .toByte
  ...
  def fillRect[T: ([x] =>> Integral[x] | Fractional[x])](
      rect: Rect[Int],
      color: Color[T]
    ): Unit = fillRect(rect, mapRGBA(color))
  ...
  def fillRect[T: ([x] =>> Integral[x] | Fractional[x])](
      ...
      h: Int,
      color: Color[T]
    ): Unit = fillRect(x, y, w, h, mapRGBA(color))
...
object Surface:
  ...
  def apply(width: Int, height: Int, format: PixelFormat = PixelFormat.RGBA32)
      : Surface = new Surface(
  ...
  def loadImage(filename: String, pixelFormat: PixelFormat | Null = null)
      : Surface =
    lazy val errPrefix =
      s"Failed to load image '$filename':${util.Properties.lineSeparator}"
    val (width, height, channels, imageData) = Using(stackPush()): stack =>
    ...
    val internal = SDL_CreateSurfaceFrom(
      width, height, internalPixelFormat, imageData, width * channels
    )
    if internal == null then
      stbi_image_free(imageData)
      sdlError()
    else new Surface(internal, Some(imageData))
  ...
  @implicitNotFound(
    "'${T}': This type cannot represent a color. Please use a numeric type."
  )
  ...
  given PixelData[Byte]:
    ...
    def get(surf: Surface)(pos: Pos): Color[Byte] = Using(stackPush()): stack =>
      val (r, g, b, a) = mallocMany(4, stack)
